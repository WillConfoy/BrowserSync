...
func Start(leader bool, window string) Node {
    ...
    mynode := Node{Leader: leader, maxX: x, maxY: y, Window: window}
    return mynode
}
...
func (node *Node) Run(addrs []string, ip string, port string) {
    // node.myaddr = "localhost:" + port
    if ip == "-1" {
        node.myaddr = GetLocalIP() + ":" + port
    } else {
        node.myaddr = ip + ":" + port
    }
    
    log.Println(node.myaddr)
	addrs = slices.DeleteFunc(addrs, func(addr string) bool {return node.myaddr == addr})
